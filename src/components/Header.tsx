...
export const Header = () => {
  ...
  const options = useMemo(() => {
    const provinces = [...new Set(rawData.map(d => d.Provincia))].sort();
    const areas = [...new Set(rawData.map(d => d['Area en la que se desempeña']))].sort();
    const populations = [...new Set(rawData.map(d => d['Población a la que pertenece']))].sort();
    
    const filteredByProv = filters.provincia.length > 0 
      ? rawData.filter(d => filters.provincia.includes(d.Provincia))
      : rawData;
    const municipalities = [...new Set(filteredByProv.map(d => d.Municipio))].sort();

    return { provinces, municipalities, areas, populations };
  }, [rawData, filters.provincia]);
